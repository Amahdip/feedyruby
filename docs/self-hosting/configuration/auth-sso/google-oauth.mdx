---
title: "Google OAuth"
description: "Configure Google OAuth for secure Single Sign-On with your FeedyRuby instance. Implement enterprise-grade authentication for your survey platform with Google credentials."
icon: "google"
---

<Note>Google OAuth is part of the FeedyRuby [Enterprise Edition](/self-hosting/advanced/license)</Note>

### Google OAuth

Integrating Google OAuth with your FeedyRuby instance allows users to log in using their Google credentials, ensuring a secure and streamlined user experience. This guide will walk you through the process of setting up Google OAuth for your FeedyRuby instance.

### Requirements

- A Google Cloud Platform (GCP) account

- A FeedyRuby instance running

### Account Deletion SSO Confirmation

For SSO-only users, FeedyRuby asks the user to type their email address and then redirects them through Google OAuth before deleting the account. FeedyRuby asks Google for an interactive login prompt, and the deletion continues only when Google returns the same linked provider account.
...
<Warning>
  If you set `DISABLE_ACCOUNT_DELETION_SSO_CONFIRMATION=1`, FeedyRuby skips this SSO identity confirmation
...
### How to connect your FeedyRuby instance to Google
...
<Steps>
  ...
  <Step title="Configure OAuth Consent Screen">
    ...
    - Fill in the necessary details and under **Authorized domains**, add the domain where your FeedyRuby instance is hosted.
  </Step>
  ...
  <Step title="Restart Your FeedyRuby Instance">
    <Note>
      Restarting your Docker containers may cause a brief period of downtime. Plan accordingly.
    </Note>
    
    - Once the environment variables have been updated, it's crucial to restart your Docker containers to apply the changes. This ensures that your FeedyRuby instance can utilize the new Google OAuth configuration for user authentication.
