...
description: "Deploy FeedyRuby on Kubernetes with the current OCI Helm chart."
icon: "circle-nodes"
---

Deploy FeedyRuby on Kubernetes using the current OCI Helm chart published from the `charts/feedyruby`
directory in the FeedyRuby repository.

<Info>
  FeedyRuby v5 self-hosting expects Hub to be part of the runtime. The chart handles that by default. Use the
...
- a public hostname for `feedyruby.webappUrl`
...
postgresql:
  enabled: false
  externalDatabaseUrl: "postgresql://user:password@your-postgres-host:5432/feedyruby"
...
FeedyRuby v5 does not support `hub.enabled=false`. Keep the default `hub.enabled=true` behavior in place.
...
Cube is part of the baseline FeedyRuby v5 stack and is bundled with the chart by default
...
The Helm chart can optionally deploy a FeedyRuby-provided Qwen runtime through vLLM. This is disabled by
default and requires GPU-capable Kubernetes nodes.

To deploy Qwen/vLLM and automatically point the FeedyRuby app at the in-cluster OpenAI-compatible endpoint:
...
If you want to deploy the bundled Qwen runtime without changing the FeedyRuby app AI configuration:
...
For a FeedyRuby 4.x to 5.0 migration, confirm the following before running the upgrade:
...
| Field                         | Description                                                   |
| ----------------------------- | ------------------------------------------------------------- |
| `feedyruby.webappUrl`        | Public base URL for the FeedyRuby app                        |
| `deployment.image.tag`        | FeedyRuby image tag override                                 |
| `hub.enabled`                 | Must stay `true` in FeedyRuby v5                             |
...
| `llm.autoConfigureApp`        | Injects FeedyRuby OpenAI-compatible env vars for bundled Qwen |
...
[charts/feedyruby/README.md](https://github.com/feedyruby/feedyruby/tree/main/charts/feedyruby).
...
```sh
helm uninstall feedyruby -n feedyruby
```

If you also want to remove in-cluster persistent volumes:

```sh
kubectl delete pvc --all -n feedyruby
```
