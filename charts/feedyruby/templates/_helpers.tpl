...
*/}}
{{- define "feedyruby.hubname" -}}
{{- $base := include "feedyruby.name" . | trunc 59 | trimSuffix "-" }}
...
*/}}
{{- define "feedyruby.cubeName" -}}
{{- $base := include "feedyruby.name" . | trunc 58 | trimSuffix "-" }}
...
*/}}
{{- define "feedyruby.labels" -}}
helm.sh/chart: {{ include "feedyruby.chart" . }}

# Selector labels
{{ include "feedyruby.selectorLabels" . }}

# Application version label
{{- with include "feedyruby.version" . }}
...
app.kubernetes.io/part-of: {{ .Values.partOfOverride | default (include "feedyruby.name" .) }}
{{- end }}
...
*/}}
{{- define "feedyruby.selectorLabels" -}}
app.kubernetes.io/name: {{ include "feedyruby.name" . }}
app.kubernetes.io/instance: {{ .Release.Name }}
app.kubernetes.io/component: {{ .Values.componentOverride | default (include "feedyruby.name" .) }}
{{- end }}
...
Usage:
{{ include "feedyruby.tplvalues.render" ( dict "value" .Values.path.to.the.Value "context" $) }}
This function allows rendering values dynamically.
*/}}
{{- define "feedyruby.tplvalues.render" -}}
...
*/}}
{{- define "feedyruby.envVarValue" -}}
{{- $value := .value -}}
{{- if kindIs "map" $value -}}
{{- include "feedyruby.tplvalues.render" (dict "value" $value "context" .context) -}}
{{- else if kindIs "invalid" $value -}}
value: ""
{{- else -}}
value: {{ include "feedyruby.tplvalues.render" (dict "value" (toString $value) "context" .context) | trim | quote }}
{{- end -}}
{{- end }}

{{- define "feedyruby.envVar" -}}
- name: {{ include "feedyruby.tplvalues.render" (dict "value" .name "context" .context) }}
  {{- include "feedyruby.envVarValue" (dict "value" .value "context" .context) | nindent 2 }}
{{- end }}

{{/*
Default OpenAI-compatible base URL for the bundled vLLM router.
*/}}
{{- define "feedyruby.llmBaseUrl" -}}
{{- if .Values.llm.feedyruby.baseUrl -}}
{{- include "feedyruby.tplvalues.render" (dict "value" .Values.llm.feedyruby.baseUrl "context" .) -}}
{{- else -}}
...
*/}}
{{- define "feedyruby.namespace" -}}
{{- default .Release.Namespace .Values.namespaceOverride -}}
{{- end -}}

{{- define "feedyruby.appSecretName" -}}
{{- printf "%s-app-secrets" (include "feedyruby.name" .) -}}
{{- end }}

{{- define "feedyruby.redisName" -}}
{{- .Values.redis.fullnameOverride | default (printf "%s-redis" (include "feedyruby.name" .)) | trunc 63 | trimSuffix "-" -}}
{{- end }}

{{- define "feedyruby.redisMasterName" -}}
{{- printf "%s-master" (include "feedyruby.redisName" .) | trunc 63 | trimSuffix "-" -}}
{{- end }}

{{- define "feedyruby.redisHeadlessName" -}}
{{- printf "%s-headless" (include "feedyruby.redisName" .) | trunc 63 | trimSuffix "-" -}}
{{- end }}

{{- define "feedyruby.redisImage" -}}
...
{{- define "feedyruby.redisSecretName" -}}
{{- .Values.redis.auth.existingSecret | default (include "feedyruby.appSecretName" .) -}}
{{- end }}

{{- define "feedyruby.redisSecretKey" -}}
{{- .Values.redis.auth.existingSecretPasswordKey | default "REDIS_PASSWORD" -}}
{{- end }}

{{- define "feedyruby.migrationJobName" -}}
{{- printf "%s-migration" (include "feedyruby.name" .) | trunc 63 | trimSuffix "-" -}}
{{- end }}

{{/*
FeedyRuby application image reference. A configured digest takes precedence over the tag.
*/}}
{{- define "feedyruby.deploymentImage" -}}
...
{{- define "feedyruby.hubSecretName" -}}
{{- default (include "feedyruby.appSecretName" .) .Values.hub.existingSecret -}}
{{- end }}

{{- define "feedyruby.hubMigrationWaitServiceAccountName" -}}
{{- printf "%s-migration-wait" (include "feedyruby.hubname" .) | trunc 63 | trimSuffix "-" -}}
{{- end }}
...
*/}}
{{- define "feedyruby.hubWorkerName" -}}
{{- $base := include "feedyruby.name" . | trunc 52 | trimSuffix "-" }}
...
*/}}
{{- define "feedyruby.hubEmbeddingsName" -}}
{{- $base := include "feedyruby.name" . | trunc 48 | trimSuffix "-" }}
...
*/}}
{{- define "feedyruby.hubEmbeddingsSecretName" -}}
{{- default (printf "%s-secret" (include "feedyruby.hubEmbeddingsName" .)) .Values.hub.embeddings.auth.existingSecret -}}
{{- end }}

{{/*
Secret used by the embeddings runtime for Hugging Face access.
*/}}
{{- define "feedyruby.hubEmbeddingsHuggingFaceSecretName" -}}
{{- default (include "feedyruby.hubEmbeddingsSecretName" .) .Values.hub.embeddings.huggingFace.existingSecret -}}
{{- end }}

{{/*
Model name Hub sends to the OpenAI-compatible embeddings endpoint.
*/}}
{{- define "feedyruby.hubEmbeddingsServedModelName" -}}
...
*/}}
{{- define "feedyruby.hubEmbeddingsBaseURL" -}}
{{- if .Values.hub.embeddings.baseUrl -}}
{{- .Values.hub.embeddings.baseUrl -}}
{{- else -}}
{{- printf "http://%s:%v/v1" (include "feedyruby.hubEmbeddingsName" .) (.Values.hub.embeddings.service.port | default .Values.hub.embeddings.port) -}}
{{- end -}}
{{- end }}
...
*/}}
{{- define "feedyruby.hubEmbeddingsApiKey" -}}
{{- $secretName := include "feedyruby.hubEmbeddingsSecretName" . }}
...
*/}}
{{- define "feedyruby.hubEmbeddingEnv" -}}
{{- $root := .root -}}
{{- if $root.Values.hub.embeddings.enabled }}
- name: EMBEDDING_PROVIDER
  value: "openai"
- name: EMBEDDING_MODEL
  value: {{ include "feedyruby.hubEmbeddingsServedModelName" $root | quote }}
- name: EMBEDDING_BASE_URL
  value: {{ include "feedyruby.hubEmbeddingsBaseURL" $root | quote }}
- name: EMBEDDING_PROVIDER_API_KEY
  valueFrom:
    secretKeyRef:
      name: {{ include "feedyruby.hubEmbeddingsSecretName" $root }}
...
*/}}
{{- define "feedyruby.hubEmbeddingEnvManaged" -}}
{{- $key := .key -}}
...
{{- define "feedyruby.postgresAdminPassword" -}}
{{- $secret := (lookup "v1" "Secret" .Release.Namespace (include "feedyruby.appSecretName" .)) }}
{{- $secretData := dig "data" dict $secret }}
...
{{- define "feedyruby.postgresUserPassword" -}}
{{- $secret := (lookup "v1" "Secret" .Release.Namespace (include "feedyruby.appSecretName" .)) }}
{{- $secretData := dig "data" dict $secret }}
...
{{- define "feedyruby.redisPassword" -}}
{{- $redisSecretName := include "feedyruby.redisSecretName" . }}
{{- $redisSecretKey := include "feedyruby.redisSecretKey" . }}
...
{{- else if eq $redisSecretName (include "feedyruby.appSecretName" .) }}
    {{- randAlphaNum 16 -}}
{{- else }}
...
{{- define "feedyruby.cronSecret" -}}
{{- $secret := (lookup "v1" "Secret" .Release.Namespace (include "feedyruby.appSecretName" .)) }}
{{- $secretData := dig "data" dict $secret }}
{{- if index $secretData "CRON_SECRET" }}
    {{- index $secretData "CRON_SECRET" | b64dec -}}
{{- else if and $secret (hasKey $secret "data") }}
    {{- fail (printf "Secret %q exists in namespace %q but is missing CRON_SECRET" (include "feedyruby.appSecretName" .) .Release.Namespace) -}}
{{- else }}
    {{- randAlphaNum 32 -}}
{{- end -}}
{{- end }}

{{- define "feedyruby.encryptionKey" -}}
{{- $secret := (lookup "v1" "Secret" .Release.Namespace (include "feedyruby.appSecretName" .)) }}
{{- $secretData := dig "data" dict $secret }}
{{- if index $secretData "ENCRYPTION_KEY" }}
    {{- index $secretData "ENCRYPTION_KEY" | b64dec -}}
{{- else if and $secret (hasKey $secret "data") }}
    {{- fail (printf "Secret %q exists in namespace %q but is missing ENCRYPTION_KEY" (include "feedyruby.appSecretName" .) .Release.Namespace) -}}
{{- else }}
    {{- randAlphaNum 32 -}}
{{- end -}}
{{- end }}

{{- define "feedyruby.nextAuthSecret" -}}
{{- $secret := (lookup "v1" "Secret" .Release.Namespace (include "feedyruby.appSecretName" .)) }}
{{- $secretData := dig "data" dict $secret }}
{{- if index $secretData "NEXTAUTH_SECRET" }}
    {{- index $secretData "NEXTAUTH_SECRET" | b64dec -}}
{{- else if and $secret (hasKey $secret "data") }}
    {{- fail (printf "Secret %q exists in namespace %q but is missing NEXTAUTH_SECRET" (include "feedyruby.appSecretName" .) .Release.Namespace) -}}
{{- else }}
    {{- randAlphaNum 32 -}}
{{- end -}}
{{- end }}

{{- define "feedyruby.hubApiKey" -}}
{{- $hubSecretName := include "feedyruby.hubSecretName" . }}
...
{{- define "feedyruby.cubejsApiSecret" -}}
{{- $secret := (lookup "v1" "Secret" .Release.Namespace (include "feedyruby.appSecretName" .)) }}
{{- $secretData := dig "data" dict $secret }}
...
{{- end -}}
{{- end }}
{{- define "feedyruby.envoy.gatewayClassName" -}}
{{- if .Values.envoy.feedyruby.gatewayClass.name -}}
{{- .Values.envoy.feedyruby.gatewayClass.name | trunc 63 | trimSuffix "-" -}}
{{- else -}}
{{- printf "%s-%s-envoy" .Release.Name (include "feedyruby.namespace" .) | trunc 63 | trimSuffix "-" -}}
{{- end -}}
{{- end }}

{{- define "feedyruby.envoy.gatewayName" -}}
{{- if .Values.envoy.feedyruby.gateway.name -}}
{{- .Values.envoy.feedyruby.gateway.name | trunc 63 | trimSuffix "-" -}}
{{- else -}}
{{- printf "%s-envoy-gateway" (include "feedyruby.name" .) | trunc 63 | trimSuffix "-" -}}
{{- end -}}
{{- end }}

{{- define "feedyruby.envoy.proxyName" -}}
{{- if .Values.envoy.feedyruby.proxy.name -}}
{{- .Values.envoy.feedyruby.proxy.name | trunc 63 | trimSuffix "-" -}}
{{- else -}}
{{- printf "%s-envoy-proxy" (include "feedyruby.name" .) | trunc 63 | trimSuffix "-" -}}
{{- end -}}
{{- end }}

{{- define "feedyruby.envoy.proxyServiceName" -}}
{{- if .Values.envoy.feedyruby.proxy.service.name -}}
{{- .Values.envoy.feedyruby.proxy.service.name | trunc 63 | trimSuffix "-" -}}
{{- else -}}
{{- printf "%s-envoy" (include "feedyruby.name" .) | trunc 63 | trimSuffix "-" -}}
{{- end -}}
{{- end }}

{{- define "feedyruby.envoy.ingressHost" -}}
{{- if .Values.envoy.feedyruby.ingress.host -}}
{{- tpl .Values.envoy.feedyruby.ingress.host $ -}}
{{- else if and .Values.ingress.hosts (gt (len .Values.ingress.hosts) 0) -}}
{{- tpl (index .Values.ingress.hosts 0).host $ -}}
{{- end -}}
{{- end }}

{{- define "feedyruby.envoy.defaultRedisUrl" -}}
