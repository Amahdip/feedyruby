...
pull_image() {
  ...
  if [[ -f /home/ubuntu/salamruby-rustfs-images.tar.gz ]]; then
    log "Loading RustFS images from fallback tarball..."
    gunzip -c /home/ubuntu/salamruby-rustfs-images.tar.gz | sudo docker load
    return 0
  fi

  log "ERROR: Could not pull ${image}. Place salamruby-rustfs-images.tar.gz in /home/ubuntu/ or fix registry access."
  return 1
}
...
if grep -q "^  rustfs:" docker-compose.yml; then
  ...
  RUSTFS_ADMIN_USER="salamruby-$(openssl rand -hex 4)"
  RUSTFS_ADMIN_PASSWORD="$(openssl rand -base64 20)"
  RUSTFS_SERVICE_USER="salamruby-service-$(openssl rand -hex 4)"
  RUSTFS_SERVICE_PASSWORD="$(openssl rand -base64 20)"
  RUSTFS_BUCKET_NAME="salamruby-uploads"
  RUSTFS_POLICY_NAME="salamruby-policy"
  RUSTFS_REGION="us-east-1"

  touch .env
  chmod 600 .env
  upsert_dotenv_var "SALAMRUBY_RUSTFS_ADMIN_USER" "$RUSTFS_ADMIN_USER" .env
  upsert_dotenv_var "SALAMRUBY_RUSTFS_ADMIN_PASSWORD" "$RUSTFS_ADMIN_PASSWORD" .env
  upsert_dotenv_var "SALAMRUBY_RUSTFS_SERVICE_USER" "$RUSTFS_SERVICE_USER" .env
  upsert_dotenv_var "SALAMRUBY_RUSTFS_SERVICE_PASSWORD" "$RUSTFS_SERVICE_PASSWORD" .env
  upsert_dotenv_var "SALAMRUBY_RUSTFS_BUCKET_NAME" "$RUSTFS_BUCKET_NAME" .env
  upsert_dotenv_var "SALAMRUBY_RUSTFS_POLICY_NAME" "$RUSTFS_POLICY_NAME" .env
  upsert_dotenv_var "SALAMRUBY_RUSTFS_REGION" "$RUSTFS_REGION" .env
  ...
  log "Configuring S3 env vars for SalamRuby app..."
  sed -i 's|# S3_ACCESS_KEY:|S3_ACCESS_KEY: "${SALAMRUBY_RUSTFS_SERVICE_USER}"|' docker-compose.yml
  sed -i 's|# S3_SECRET_KEY:|S3_SECRET_KEY: "${SALAMRUBY_RUSTFS_SERVICE_PASSWORD}"|' docker-compose.yml
  sed -i 's|# S3_REGION:|S3_REGION: "${SALAMRUBY_RUSTFS_REGION}"|' docker-compose.yml
  sed -i 's|# S3_BUCKET_NAME:|S3_BUCKET_NAME: "${SALAMRUBY_RUSTFS_BUCKET_NAME}"|' docker-compose.yml
  ...
  rustfs:
    ...
    command: /data
    environment:
      RUSTFS_ACCESS_KEY: "\${SALAMRUBY_RUSTFS_ADMIN_USER}"
      RUSTFS_SECRET_KEY: "\${SALAMRUBY_RUSTFS_ADMIN_PASSWORD}"
      RUSTFS_ADDRESS: ":9000"
    volumes:
      - rustfs-data:/data
    labels:
  ...
  rustfs-init:
    ...
    depends_on:
      - rustfs
    environment:
      RUSTFS_ADMIN_USER: "\${SALAMRUBY_RUSTFS_ADMIN_USER}"
      RUSTFS_ADMIN_PASSWORD: "\${SALAMRUBY_RUSTFS_ADMIN_PASSWORD}"
      RUSTFS_SERVICE_USER: "\${SALAMRUBY_RUSTFS_SERVICE_USER}"
      RUSTFS_SERVICE_PASSWORD: "\${SALAMRUBY_RUSTFS_SERVICE_PASSWORD}"
      RUSTFS_BUCKET_NAME: "\${SALAMRUBY_RUSTFS_BUCKET_NAME}"
      RUSTFS_POLICY_NAME: "\${SALAMRUBY_RUSTFS_POLICY_NAME}"
...
log "Test upload from Organization settings after salamruby restarts."
