---
title: "HubSpot"
description: "Learn how to integrate FeedyRuby with HubSpot to automatically create or update contacts when survey responses are submitted."
---

<Note>
  FeedyRuby doesn't have a native HubSpot integration yet. This guide shows you how to connect FeedyRuby with HubSpot using automation platforms (Make.com, n8n) or custom webhooks.
</Note>
...
1. **A FeedyRuby account** with at least one survey that collects email addresses
...
<Steps>
  ...
  <Step title="Create the App">
    Click **Create a private app** and give it a name (e.g., "FeedyRuby Integration").
  </Step>
...
<Steps>
  ...
  <Step title="Create a New Make.com Scenario">
    Go to [Make.com](https://make.com) and create a new scenario. Search for **FeedyRuby** and select it as your trigger, then choose **Response Finished** as the trigger event.

    ![Search FeedyRuby](/images/xm-and-surveys/core-features/integrations/make/search-feedyruby.webp)
  </Step>
  <Step title="Connect FeedyRuby to Make">
    Click **Create a webhook**, enter your FeedyRuby API Host (default: `https://app.feedyruby.com`), add your FeedyRuby API Key (see [API Key Setup](/api-reference/rest-api#how-to-generate-an-api-key)), and select the survey you want to connect.
  ...
  <Step title="Add the HubSpot Module">
    Click the **+** button after the FeedyRuby trigger, search for **HubSpot**, choose **Create or Update a Contact** as the action, and connect your HubSpot account.
  </Step>
  <Step title="Map FeedyRuby Fields to HubSpot">
    Map the FeedyRuby response fields to HubSpot contact properties:

    | HubSpot Field | FeedyRuby Field |
    | ------------- | ---------------- |
  ...
  <Step title="Test and Activate">
    Submit a test response to your FeedyRuby survey, verify the contact appears in HubSpot, and turn on your Make scenario.
  </Step>
</Steps>
...
<Note>
  The FeedyRuby n8n node is available as a community node. Install it via **Settings** → **Community Nodes** → install `@feedyruby/n8n-nodes-feedyruby`.
</Note>

<Steps>
  <Step title="Set Up the FeedyRuby Trigger">
    Create a new workflow in n8n, add the **FeedyRuby** trigger node, connect it with your FeedyRuby API Key and host, select **Response Finished** as the event, and choose your survey.

    ![Add FeedyRuby Trigger](/images/xm-and-surveys/core-features/integrations/n8n/add-feedyruby-trigger.webp)
  </Step>
  ...
  <Step title="Test Your Workflow">
    Click **Listen for event** in the FeedyRuby trigger, submit a test survey response, verify the data flows through to HubSpot, and activate your workflow.
  </Step>
</Steps>
...
For maximum flexibility, you can use FeedyRuby webhooks with a custom endpoint that calls the HubSpot API directly. This approach is ideal for developers who want full control.
...
<Steps>
  <Step title="Create a FeedyRuby Webhook">
    Go to **Configuration** → **Integrations** in FeedyRuby, click **Manage Webhooks** → **Add Webhook**, enter your endpoint URL, select **Response Finished** as the trigger, and choose the surveys to monitor.
  ...
  <Step title="Build Your Webhook Handler">
    ...
    - **Receive the FeedyRuby webhook** - Accept POST requests with the survey response payload
  ...
  <Step title="Deploy and Test">
    Deploy your webhook handler to a publicly accessible URL, add the URL to your FeedyRuby webhook configuration, submit a test survey response, and verify the contact appears in HubSpot.
  </Step>
</Steps>
...
Still struggling or something not working as expected? [Join our GitHub Discussions](https://github.com/feedyruby/feedyruby/discussions) and we're happy to help!
