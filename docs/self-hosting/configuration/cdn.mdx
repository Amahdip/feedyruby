---
title: "CDN"
description: "Use a CDN in front of public FeedyRuby traffic without serving stale survey assets after upgrades."
icon: "globe"
---

### Why a CDN is recommended

A CDN is recommended for production self-hosting when you expose FeedyRuby surveys publicly. It helps you:
...
You can run FeedyRuby without a CDN, but it is usually the better production setup once surveys are reachable from
the public internet.
...
Configure your CDN to proxy requests to your FeedyRuby origin while preserving the original `Host` header and HTTPS
...
<Warning>
  Do not enable a blanket "cache everything" rule for all FeedyRuby routes. That can cause stale survey pages,
...
"Ignore origin headers". Those features can easily keep old FeedyRuby responses alive longer than intended.

## Important detail for upgrades

FeedyRuby serves the survey runtime bundle from a stable public path:
...
FeedyRuby version, respondents may receive outdated survey code even though the rest of the app has already been
updated.
...
Every time you update FeedyRuby, include CDN cache handling in your rollout:

<Steps>
  <Step title="Deploy the new FeedyRuby version">
    Update FeedyRuby as usual by following the [Migration Guide](/self-hosting/advanced/migration) or your normal
    deployment process.
  </Step>
  <Step title="Purge or revalidate public assets">
    Purge the CDN cache for at least `/js/*`, `/_next/*`, and `/css/*`. If your CDN
    supports targeted purges, make sure `/js/surveys.umd.cjs` is included every time.
  </Step>
...
1. Put only the public FeedyRuby domain behind the CDN.
...
5. Add a purge step to every FeedyRuby deployment or upgrade.
