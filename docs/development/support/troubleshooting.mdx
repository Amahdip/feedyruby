...
This usually happens when the FeedyRuby Widget isn't correctly or completely built.

```bash
pnpm build --filter=@feedyruby/js
...
```bash helloWorld.js
pnpm dev --filter=@feedyruby/web...
```

It’s better to use a single terminal with `pnpm dev` rather than having multiple open (one with the FeedyRuby app and one with the demo).
