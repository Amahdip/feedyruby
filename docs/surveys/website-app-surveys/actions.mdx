---
title: "Actions"
description: "Actions are predefined events within your app that prompt FeedyRuby to display a survey when triggered. These are detected by the FeedyRuby widget, which then presents the appropriate survey based on your predefined settings."
icon: "code"
---
...
<Step title="FeedyRuby widget detects action">
...
<Step title="Check for survey trigger">
  If a survey is set to trigger on that action, FeedyRuby checks if the user or visitor qualifies.
</Step>

  <Step title="Check for user or visitor qualification">
    If the user or visitor already filled out a survey in the past couple of days (see [Workspace-wide Survey Cooldown](/xm-and-surveys/surveys/website-app-surveys/recontact#workspace-wide-survey-cooldown)), the survey will not be shown to prevent survey fatigue.

    Also, if this user or visitor has seen this specific survey before, FeedyRuby might not show it as this is dependent on the [Recontact Options](/xm-and-surveys/surveys/website-app-surveys/recontact).
...
FeedyRuby offers an intuitive No-Code interface that allows you to configure actions without needing to write any code.
...
<Steps>
  <Step title="Visit the Settings tab and click on `User Actions`">
    ![Action overview on FeedyRuby Open Source Survey Solution](/images/xm-and-surveys/surveys/website-app-surveys/actions/actions-view.webp "Action overview on FeedyRuby Open Source Survey Solution")
  </Step>
...
<Steps>
  <Step title="Configure action in FeedyRuby">
    First, add the action via the FeedyRuby web interface to make it available for survey configuration:
...
![Add a code action to open source in app survey](/images/xm-and-surveys/surveys/website-app-surveys/actions/code-action.webp "Add a code action to open source in app survey")
  ...
  <Step title="Add action tracking to your code">
    Use feedyruby.track() to send an action event to FeedyRuby:

    ```
    feedyruby.track("action");
    ```
    ...
    ```javascript
    const handleClick = () => {
      feedyruby.track("Button Clicked");
    };
