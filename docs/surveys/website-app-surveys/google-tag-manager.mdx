...
description: "Deploy FeedyRuby surveys through GTM without modifying your website code."
icon: "tags"
---
...
- Your FeedyRuby **Workspace ID** (Settings > Connect Your App)
- Your **App URL**: `https://app.feedyruby.com` (or your self-hosted URL)
...
<Steps>
  <Step title="Create a Custom HTML tag in GTM">
    1. Create a new tag with preferred name e.g. "FeedyRuby Intercept Surveys"
...
!function(){
    var appUrl = "https://app.feedyruby.com"; // REPLACE ONLY IF YOUR SELF-HOST
    ...
    t.onload=function(){
        window.feedyruby && window.feedyruby.setup({
...
![Add GTM Custom HTML tag](/images/xm-and-surveys/surveys/website-app-surveys/google-tag-manager/create-a-tag.webp)
  ...
  <Step title="Test">
    ...
    3. Add `?feedyrubyDebug=true` to the URL to see test logs in browser console (see [Debugging Mode](/xm-and-surveys/surveys/website-app-surveys/framework-guides#debugging-feedyruby-integration) for more details)
  </Step>
</Steps>
...
<Steps>
  ...
  <Step title="Create identification tag">
    New Custom HTML tag named "FeedyRuby - User":

```html
<script>
(function() {
    var check = setInterval(function() {
        if (window.feedyruby && window.feedyruby.setUserId) {
            clearInterval(check);
            var userId = {{User ID}};
            if (userId) {
                window.feedyruby.setUserId(userId);
                var attrs = {};
                if ({{User Email}}) attrs.email = {{User Email}};
                if ({{User Plan}}) attrs.plan = {{User Plan}};
                if (Object.keys(attrs).length) {
                    window.feedyruby.setAttributes(attrs);
                }
            }
        }
    }, 100);
...
```
  ...
  <Step title="Set trigger and push data">
    ...
    4. Attach this trigger to your "FeedyRuby - User" tag
    5. Save and publish
...
<Steps>
  <Step title="Create code action in FeedyRuby">
    Add code action via FeedyRuby UI
...
```html
<script>
if (window.feedyruby && window.feedyruby.track) {
    window.feedyruby.track({{Event Name}});
}
</script>
```
...
- Add `?feedyrubyDebug=true` to your URL
...
**Events not tracking?**
- Confirm `window.feedyruby` exists before calling track
- Match event names exactly with FeedyRuby action names
- Check timing - FeedyRuby must be initialized first

## Need Help?

- [GitHub Discussions](https://github.com/feedyruby/feedyruby/discussions)
