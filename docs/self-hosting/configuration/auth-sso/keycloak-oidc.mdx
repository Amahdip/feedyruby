---
title: "Keycloak OIDC"
description: "Configure Keycloak as an OpenID Connect (OIDC) provider for secure Single Sign-On with your self-hosted FeedyRuby instance."
icon: "key-skeleton-left-right"
---

<Note>Keycloak OIDC is part of the FeedyRuby [Enterprise Edition](/self-hosting/advanced/license)</Note>

Integrating Keycloak with your FeedyRuby instance allows users to log in using their existing Keycloak credentials via OpenID Connect. This guide walks you through setting up a Keycloak OIDC client and configuring FeedyRuby to use it, including automatic user provisioning into a default team.

### Requirements

- A running Keycloak instance with a configured realm and users.
- A self-hosted FeedyRuby instance with a valid [Enterprise license](/self-hosting/advanced/license).
- The FeedyRuby callback URI: `{WEBAPP_URL}/api/auth/callback/openid`
...
<Steps>
  <Step title="Create a new client in Keycloak">
    ...
    - Choose a **Client ID** (e.g., `feedyruby`) and click **Next**.
  </Step>
  ...
  <Step title="Configure client authentication">
    ...
    <Note>
      FeedyRuby uses PKCE (Proof Key for Code Exchange) for added security. Modern Keycloak versions (21+) support PKCE automatically for confidential clients. No additional PKCE configuration is needed.
    </Note>
  </Step>

  <Step title="Set the redirect URI">
    - Under **Valid redirect URIs**, add your FeedyRuby callback URI:

    ```
    https://your-feedyruby-domain.com/api/auth/callback/openid
    ```

    Replace `your-feedyruby-domain.com` with your actual FeedyRuby URL.
  ...
  <Step title="Copy the client secret">
    ...
    - Copy the **Client secret** value — you will need this for the FeedyRuby configuration.
  </Step>
  ...
  <Step title="Configure FeedyRuby environment variables">
    Add the following environment variables to your FeedyRuby `.env` file or `docker-compose.yml`:

    ```yml FeedyRuby Env for Keycloak OIDC
    OIDC_CLIENT_ID=feedyruby
    ...
    | --- | --- |
    | `OIDC_CLIENT_ID` | The Client ID you chose in Keycloak (e.g., `feedyruby`). |
    | `OIDC_CLIENT_SECRET` | The client secret from the **Credentials** tab. |
    | `OIDC_ISSUER` | Your Keycloak realm URL (see step above). |
    | `OIDC_DISPLAY_NAME` | The label shown on the FeedyRuby login button. |
    | `OIDC_SIGNING_ALGORITHM` | Optional. Keycloak and FeedyRuby both default to `RS256`. Only set this if your realm uses a different algorithm. |
  </Step>

  <Step title="Restart and test">
    - Restart your FeedyRuby instance.
...
By default, new SSO users must be invited by an organization admin before they can access FeedyRuby. To allow Keycloak users to join automatically and be assigned to a team, configure the following additional environment variables:
...
1. Log in to FeedyRuby as an admin.
...
<Note>
  The Teams feature requires the **Role-Based Access Control (RBAC)** add-on on your Enterprise license. If you don't see the Teams section in Organization Settings, contact [FeedyRuby support](mailto:support@feedyruby.com) to enable it.
</Note>
...
2. FeedyRuby creates a new user account.
3. The user is automatically added as a **member** of the organization that owns the specified team.
4. The user is assigned to the default team, granting them access to all Workspaces associated with that team.
...
<AccordionGroup>
  ...
  <Accordion title="Users see 'No membership found' after login">
    FeedyRuby determines the organization automatically from the `AUTH_SSO_DEFAULT_TEAM_ID` — you do not need to set a separate organization ID. If this error appears:
  ...
  <Accordion title="OIDC login button does not appear">
    ...
    - Ensure your FeedyRuby instance has a valid Enterprise license.
    - Restart the FeedyRuby container after adding the environment variables.
  </Accordion>
  ...
  <Accordion title="Authentication fails with a redirect error">
    ...
    - Check that the OIDC issuer URL is reachable from your FeedyRuby server. You can test with:
    ```bash
