...
Enabling the Airtable Integration in a self-hosted environment requires creating an airtable account and changing the environment variables of your FeedyRuby instance.
...
* Now just copy **Client ID** for your integration & add it to your **FeedyRuby environment variables** as in the docker compose file:

* `AIRTABLE_CLIENT_ID`

Voila! You have successfully enabled the Airtable integration in your self-hosted FeedyRuby instance. Now you can follow the steps mentioned in [Airtable Integration with FeedyRuby](/xm-and-surveys/core-features/integrations/airtable) section to link an Airtable with FeedyRuby.
