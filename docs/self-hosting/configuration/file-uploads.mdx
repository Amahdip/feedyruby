...
description: "Configure file storage for survey images, file uploads, and Workspace assets in your self-hosted FeedyRuby instance"
icon: "upload"
---

FeedyRuby requires S3-compatible storage for file uploads. You can use external cloud storage services or the bundled RustFS option for a self-hosted solution.

## Why Configure File Uploads?

Setting up file storage enables important features in FeedyRuby, including:

- Adding images to surveys (questions, backgrounds, logos)
- 'File Upload' and 'Picture Selection' question types
- Workspace logos and branding
...
FeedyRuby supports S3-compatible storage with two main configurations:
...
RustFS provides a self-hosted S3-compatible storage solution that runs alongside FeedyRuby. This option:

- Runs in a Docker container alongside FeedyRuby
...
When using the FeedyRuby installation script, you'll be prompted to configure file uploads:
...
Choose this option for a self-hosted S3-compatible storage that runs alongside FeedyRuby:
...
- Generate separate RustFS admin and FeedyRuby service credentials
- Create the storage bucket
- Configure SSL certificates for the files subdomain
- Configure Traefik routing for the subdomain
- Store the generated RustFS credentials in `./feedyruby/.env` with restricted permissions
...
S3_REGION=us-east-1
S3_BUCKET_NAME=my-feedyruby-uploads
...
S3_REGION=nyc3
S3_BUCKET_NAME=my-feedyruby-space
...
FeedyRuby uses the S3 [POST Object](https://docs.aws.amazon.com/AmazonS3/latest/API/RESTObjectPOST.html)
operation (presigned POST) for uploads. Your object storage provider must support this operation. Providers
that do not implement POST Object are not compatible with FeedyRuby uploads. For example, Backblaze B2's
S3‑compatible API currently does not support POST Object and therefore will not work with FeedyRuby file
uploads.
...
- **Bucket**: Automatically created `feedyruby-uploads` bucket
...
🗄️  RustFS Storage Setup Complete:
   • Access Key: feedyruby-service-a1b2c3d4
   • Bucket: feedyruby-uploads (✅ automatically created)
```
...
services:
  feedyruby:
    image: ghcr.io/feedyruby/feedyruby:latest
    environment:
...
The following IAM policy grants the minimum required permissions for FeedyRuby to function correctly. This policy is also used in the bundled RustFS integration:
...
2. **Restricted Write Access**: Only your FeedyRuby instance should be able to upload files
3. **CORS Configuration**: Allow requests from your FeedyRuby domain
...
```json
{
  "Statement": [
    ...
    },
    {
      ...
      "Effect": "Allow",
      "Principal": {
        "AWS": "arn:aws:iam::123456789012:user/feedyruby-service"
      },
      "Resource": "arn:aws:s3:::your-bucket-name/*",
      "Sid": "AllowFeedyRubyWrite"
    }
  ],
  "Version": "2012-10-17"
}
...
<Note>
  ...
  `arn:aws:iam::123456789012:user/feedyruby-service` with the ARN of your IAM user. This policy allows public
  read access only to specific paths while restricting write access to your FeedyRuby service user.
</Note>

### S3 CORS Configuration

CORS (Cross-Origin Resource Sharing) must be configured on your S3 bucket to allow FeedyRuby to upload files using presigned POST URLs. Without proper CORS configuration, file uploads from the browser will fail.
...
<Note>
  For production environments, consider restricting `AllowedOrigins` to your specific FeedyRuby domain(s)
...
**Bundled FeedyRuby setup** (the [one-click script](/self-hosting/setup/one-click), driven by `docker/rustfs-init.sh`): CORS is configured for you. The bootstrap applies a per-bucket policy via `mc cors set`, scoped to your FeedyRuby domain — nothing else to do.

**Standalone RustFS** (managed separately from FeedyRuby): set `RUSTFS_CORS_ALLOWED_ORIGINS` on the RustFS server to the origin(s) you serve FeedyRuby from — your `WEBAPP_URL` — comma-separated for multiple values:
...
<Note>
  ...
  this variable is most likely already set to a value that excludes your FeedyRuby origin — add your origin to it
...
- Ship RustFS and FeedyRuby container logs into your normal logging and alerting stack; the bundled setup
...
5. We use S3 presigned URLs for uploads. Make sure your CORS policy allows presigned URL uploads; otherwise, uploads will fail.
   Some providers (e.g., Hetzner’s object storage) [require a specific CORS configuration](https://github.com/feedyruby/feedyruby/discussions/6641#discussioncomment-14574048).
   If you’re using the bundled RustFS setup, this is already configured for you.
6. **The file appears in your storage but FeedyRuby still reports “upload failed.”** This is the classic CORS symptom — the browser uploaded the file successfully but could not read the response. For RustFS, set `RUSTFS_CORS_ALLOWED_ORIGINS` to include your FeedyRuby origin (see [RustFS CORS Configuration](#rustfs-cors-configuration)).
...
1. **Admin Panel**: Try uploading a Workspace logo in the Workspace settings
...
```bash
# Check FeedyRuby logs
docker compose logs feedyruby

# Check RustFS logs (if using bundled RustFS)
docker compose logs rustfs
```

For additional help, join the conversation on [GitHub Discussions](https://github.com/feedyruby/feedyruby/discussions).
