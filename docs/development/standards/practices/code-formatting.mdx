...
description: "Standards for consistent code formatting across the FeedyRuby codebase"
icon: code
---
...
### Prettier Config
FeedyRuby uses a standardized Prettier configuration defined in `@feedyruby/config-prettier`:
```javascript
module.exports = {
  bracketSpacing: true,
...
- Consistent use of `tsconfig.json` extending from `@feedyruby/config-typescript`
...
```json
{
  ...
  "extends": "@feedyruby/config-typescript/js-library.json",
...
Each package extends from `@feedyruby/eslint-config` with specific presets:
...
These standards ensure consistency across the FeedyRuby codebase while maintaining high code quality and developer productivity.
