---
title: "Docker Setup"
description: "Set up FeedyRuby quickly using our ready-to-use Docker image."
icon: "docker"
---
...
<Info>
  Starting with FeedyRuby v5, the production Docker Compose stack includes FeedyRuby Hub and Cube as part of
...
1. **Create a New Directory for FeedyRuby**

   Open a terminal and run the following commands to create and enter a new directory for FeedyRuby:

   ```bash
   mkdir feedyruby-quickstart && cd feedyruby-quickstart
   ```
...
1. **Download the Docker Files**
   ...
   ```bash
   mkdir -p cube/schema
   curl -o docker-compose.yml https://raw.githubusercontent.com/feedyruby/feedyruby/stable/docker/docker-compose.yml
   curl -o cube/cube.js https://raw.githubusercontent.com/feedyruby/feedyruby/stable/docker/cube/cube.js
   curl -o cube/schema/FeedbackRecords.js https://raw.githubusercontent.com/feedyruby/feedyruby/stable/docker/cube/schema/FeedbackRecords.js
   ```

1. **Generate Hub and Cube Secrets**

   FeedyRuby Hub and Cube each require a shared secret. Create `.env` with both:
...
1. **Generate Hub API Key**
   ...
   FeedyRuby v5 requires a Hub API key for the bundled Hub service.
   ...
   <Info>
     ...
     change that value if your FeedyRuby app needs to reach Hub at a different address. If your deployment
...
1. **Start the Docker Setup**
   ...
   Now, you're ready to run FeedyRuby with Docker. Use the command below to start FeedyRuby together with
   PostgreSQL, Redis, FeedyRuby Hub, and Cube.
...
1. **Open FeedyRuby in Your Browser**

   Once the setup is running, open [**http://localhost:3000**](http://localhost:3000) in your browser to access FeedyRuby. The first time you visit, you'll see a setup wizard. Follow the steps to create your first user and start using FeedyRuby.

<Note>
  The bundled Docker stack keeps FeedyRuby Hub and Cube internal to the compose network. The app reaches them
  through `http://hub:8080` and `http://cube:4000`.
</Note>

<Info>
  If you use the one-click Traefik setup, FeedbackRecords are available on the FeedyRuby origin at
  `/api/v3/feedbackRecords` and `/v1/feedback-records`. Custom Docker reverse proxies need equivalent wiring:
  run gateway auth against the FeedyRuby app, rewrite `/api/v3/feedbackRecords` to Hub's
  `/v1/feedback-records`, and inject `Authorization: Bearer <HUB_API_KEY>` only on the Hub-bound hop.
</Info>

## Update

Please take a look at our [migration guide](/self-hosting/advanced/migration) for version specific steps to update FeedyRuby.

<Info>
  For a major migration such as FeedyRuby 4.x to 5.0, update your compose structure and configuration first.
...
1. Pull the latest FeedyRuby image

   ```bash
   docker compose pull
   ```

1. Stop the FeedyRuby stack

   ```bash
   docker compose down
   ```

1. Re-start the FeedyRuby stack with the updated image
...
RustFS provides S3-compatible object storage for file uploads in FeedyRuby. If you want to enable features
like image uploads, survey file uploads, or custom logos, you can run RustFS alongside FeedyRuby while
...
starts RustFS and auto-creates the `feedyruby` bucket.
...
1. **Start the local stack**
   ...
   bootstrap job, FeedyRuby Hub, and a local Cube instance for analytics testing.
...
4. **Verify uploads**
   ...
   After uploading a file in FeedyRuby, open http://localhost:9001 and navigate to **Buckets → feedyruby**
...
services:
  ...
  rustfs:
    ...
    environment:
      ...
      # CORS for browser uploads: set to your FeedyRuby origin(s), comma-separated.
...
```bash
FEEDYRUBY_RUSTFS_ADMIN_USER=feedyruby-root
FEEDYRUBY_RUSTFS_ADMIN_PASSWORD=change-this-secure-password
FEEDYRUBY_RUSTFS_SERVICE_USER=feedyruby-service
FEEDYRUBY_RUSTFS_SERVICE_PASSWORD=change-this-service-password
FEEDYRUBY_RUSTFS_BUCKET_NAME=feedyruby
FEEDYRUBY_RUSTFS_POLICY_NAME=feedyruby-policy
FEEDYRUBY_RUSTFS_REGION=us-east-1
```

Then configure FeedyRuby to use the RustFS service credentials:

```bash
S3_ACCESS_KEY="${FEEDYRUBY_RUSTFS_SERVICE_USER}"
S3_SECRET_KEY="${FEEDYRUBY_RUSTFS_SERVICE_PASSWORD}"
S3_REGION="${FEEDYRUBY_RUSTFS_REGION}"
S3_BUCKET_NAME="${FEEDYRUBY_RUSTFS_BUCKET_NAME}"
...
- **Health check**: From the FeedyRuby container:

  ```bash
  docker compose exec feedyruby sh -c 'wget -O- http://rustfs:9000/health'
  ```
...
- CORS configuration scoped to your FeedyRuby domain
- Rate limiting middleware
- Separate RustFS admin and FeedyRuby service credentials
- A `rustfs-init` job that creates the bucket and access policy

The production setup from [feedyruby.sh](https://github.com/feedyruby/feedyruby/blob/main/docker/feedyruby.sh)
...
<Note>
  ...
  storage or a dedicated RustFS deployment managed separately from FeedyRuby.
</Note>
...
```bash
[+] Running 9/16
⠹ feedyruby 15 layers [⣿⣤⣿⣿⣿⣿⣿⣿⣿⣿⠀⠀⠀⠀⠀] 29.78MB/47.76MB Pulling                                                           13.3s
...
[+] Running 2/2
✔ Container feedyruby-quickstart-postgres-1    Created                                                                       0.0s
✔ Container feedyruby-quickstart-feedyruby-1  Created                                                                       0.0s
```

And at the tail of the output, you should see this:

```bash
feedyruby-quickstart-feedyruby-1  | All migrations have been successfully applied.
feedyruby-quickstart-feedyruby-1  |
feedyruby-quickstart-feedyruby-1  | - info Loaded env from /home/nextjs/apps/web/.env
feedyruby-quickstart-feedyruby-1  | Listening on port 3000 url: http://<random-string>:3000
```
