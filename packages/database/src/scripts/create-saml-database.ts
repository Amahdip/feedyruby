...
const createSamlDatabase = async (): Promise<void> => {
  ...
  const defaultDatabaseUrl = samlDatabaseUrl.replace(`/${dbName}`, "/postgres");
  const prisma = new PrismaClient({ adapter: createPrismaPgAdapter(defaultDatabaseUrl).adapter });
