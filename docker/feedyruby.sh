...
# source template for the generated rustfs-init.sh in docker/feedyruby.sh for
...
if [ -n "${RUSTFS_CORS_ALLOWED_ORIGINS:-}" ]; then
  ...
  cors_file="/tmp/feedyruby-cors.xml"
...
cat > /tmp/feedyruby-policy.json << EOF
{
  "Version": "2012-10-17",
  "Statement": [
    {
...
if ! mc admin policy info rustfs "$RUSTFS_POLICY_NAME" >/dev/null 2>&1; then
  mc admin policy create rustfs "$RUSTFS_POLICY_NAME" /tmp/feedyruby-policy.json || \
    mc admin policy add rustfs "$RUSTFS_POLICY_NAME" /tmp/feedyruby-policy.json
...
write_rustfs_env_file() {
  ...
  upsert_dotenv_var "FEEDYRUBY_RUSTFS_ADMIN_USER" "$rustfs_admin_user" "$env_file"
  upsert_dotenv_var "FEEDYRUBY_RUSTFS_ADMIN_PASSWORD" "$rustfs_admin_password" "$env_file"
  upsert_dotenv_var "FEEDYRUBY_RUSTFS_SERVICE_USER" "$rustfs_service_user" "$env_file"
  upsert_dotenv_var "FEEDYRUBY_RUSTFS_SERVICE_PASSWORD" "$rustfs_service_password" "$env_file"
  upsert_dotenv_var "FEEDYRUBY_RUSTFS_BUCKET_NAME" "$rustfs_bucket_name" "$env_file"
  upsert_dotenv_var "FEEDYRUBY_RUSTFS_POLICY_NAME" "$rustfs_policy_name" "$env_file"
  upsert_dotenv_var "FEEDYRUBY_RUSTFS_REGION" "us-east-1" "$env_file"
}

install_feedyruby() {
  # Friendly welcome
  echo "🧱 Welcome to the FeedyRuby Setup Script"
  echo ""
  echo "🛸 Fasten your seatbelts! We're setting up your FeedyRuby environment on your $ubuntu_version server."
  echo ""
  ...
  echo "🎉 Hooray! Docker is all set and ready to go. You're now ready to run your FeedyRuby instance!"

  mkdir -p feedyruby && cd feedyruby
  echo "📁 Created FeedyRuby Quickstart directory at ./feedyruby."
  ...
  if [[ $https_setup == "n" ]]; then
    echo "You have chosen not to set up HTTPS certificate for your domain. Please make sure to set up HTTPS on your own. You can refer to the FeedyRuby documentation(https://feedyruby.com/docs/self-hosting/custom-ssl) for more information."
...
EOT
  ...
  echo "📥 Downloading docker-compose.yml from FeedyRuby GitHub repository..."
  curl -fsSL -o docker-compose.yml https://raw.githubusercontent.com/feedyruby/feedyruby/stable/docker/docker-compose.yml
  mkdir -p cube/schema
  echo "📥 Downloading Cube.js configuration for XM Suite v5 analytics..."
  curl -fsSL -o cube/cube.js https://raw.githubusercontent.com/feedyruby/feedyruby/stable/docker/cube/cube.js
  curl -fsSL -o cube/schema/FeedbackRecords.js https://raw.githubusercontent.com/feedyruby/feedyruby/stable/docker/cube/schema/FeedbackRecords.js
...
EOF
  ...
  # Step 1: Add Traefik labels to feedyruby service
  awk -v domain_name="$domain_name" -v hsts_enabled="$hsts_enabled" '
/feedyruby:/,/^ *$/ {
    if ($0 ~ /<<: \*environment$/) {
        print "    labels:"
        print "      - \"traefik.enable=true\""
        print "      - \"traefik.http.routers.feedyruby.rule=Host(`" domain_name "`)\""
        print "      - \"traefik.http.routers.feedyruby.entrypoints=websecure\""
        print "      - \"traefik.http.routers.feedyruby.tls=true\""
        print "      - \"traefik.http.routers.feedyruby.tls.certresolver=default\""
        print "      - \"traefik.http.services.feedyruby.loadbalancer.server.port=3000\""
        ...
        print "      - \"traefik.http.routers.feedback-records-token.service=feedyruby\""
        ...
        if (hsts_enabled == "y") {
            ...
            print "      - \"traefik.http.routers.feedyruby_http.entrypoints=web\""
            print "      - \"traefik.http.routers.feedyruby_http.rule=Host(`" domain_name "`)\""
            ...
            print "      - \"traefik.http.routers.feedback-records-token-http.service=feedyruby\""
...
in_hub && /^  [A-Za-z0-9_-]+:/ && !/^  hub:/ { in_hub = 0 }
{
    if (in_hub && !inserted && $0 ~ /^    environment:/) {
        ...
        print "      - \"traefik.http.middlewares.feedback-records-auth.forwardauth.address=http://feedyruby:3000/api/traefik-auth/feedback-records\""
...
' docker-compose.yml >tmp.yml && mv tmp.yml docker-compose.yml
  ...
  # Step 2: Ensure feedyruby waits for rustfs-init to complete successfully (mapping depends_on)
...
echo "🔗 To edit more variables and deeper config, go to the feedyruby/docker-compose.yml, edit the file, and restart the container!"
...
if [[ $rustfs_storage == "y" ]]; then
    ...
    echo "   • Generated credentials stored in ./feedyruby/.env (permissions set to 600)"
    echo ""
fi

echo "🎉 All done! Please setup your FeedyRuby instance by visiting your domain at https://$domain_name. You can check the status of FeedyRuby & Traefik with 'cd feedyruby && sudo docker compose ps.'"
...
uninstall_feedyruby() {
  echo "🗑️ Preparing to Uninstalling FeedyRuby..."
  read -p "Are you sure you want to uninstall FeedyRuby? This will delete all the data associated with it! (yes/no): " uninstall_confirmation
  uninstall_confirmation=$(echo "$uninstall_confirmation" | tr '[:upper:]' '[:lower:]')
  if [[ $uninstall_confirmation == "yes" ]]; then
    cd feedyruby
    sudo docker compose down
    cd ..
    sudo rm -rf feedyruby
    echo "🛑 FeedyRuby uninstalled successfully!"
  else
    echo "❌ Uninstalling FeedyRuby has been cancelled."
  fi
}

stop_feedyruby() {
  echo "🛑 Stopping FeedyRuby..."
  cd feedyruby
  sudo docker compose down
  echo "🎉 FeedyRuby instance stopped successfully!"
}

update_feedyruby() {
  echo "🔄 Updating FeedyRuby..."
  cd feedyruby
  sudo docker compose pull
  sudo docker compose down
  sudo docker compose up -d
  echo "🎉 FeedyRuby updated successfully!"
  echo "🎉 Check the status of FeedyRuby & Traefik with 'cd feedyruby && sudo docker compose logs.'"
}

restart_feedyruby() {
  echo "🔄 Restarting FeedyRuby..."
  cd feedyruby
  sudo docker compose restart
  echo "🎉 FeedyRuby restarted successfully!"
}

get_logs() {
  echo "📃 Getting FeedyRuby logs..."
  cd feedyruby
...
if [[ "${BASH_SOURCE[0]}" == "$0" ]]; then
  ...
  uninstall)
    ...
    ;;
  *)
    echo "🚀 Executing default step of installing FeedyRuby"
    install_feedyruby
    ;;
  esac
fi
