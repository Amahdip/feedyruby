---
title: "Rate Limiting"
description: "Current request rate limits in FeedyRuby"
icon: "timer"
---

FeedyRuby applies request rate limits to protect against abuse and keep API usage fair.

Starting with FeedyRuby v5, rate limiting is split across two layers:
...
<Warning>
  ...
  protected by Envoy Gateway. If you self-host FeedyRuby without Envoy or an equivalent edge rate limiter,
...
Before upgrading to FeedyRuby v5:
...
These limits are expected to be enforced at the gateway layer in FeedyRuby v5 and later:
...
These are the limits that still run inside the FeedyRuby application server:
