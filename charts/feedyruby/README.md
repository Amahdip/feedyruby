...
A Helm chart for FeedyRuby with PostgreSQL, Valkey

**Homepage:** <https://feedyruby.com/docs/self-hosting/setup/kubernetes>

## Maintainers

| Name       | Email                 | Url |
| ---------- | --------------------- | --- |
| FeedyRuby | <info@feedyruby.com> |     |
...
The chart can optionally deploy FeedyRuby behind Envoy Gateway with a dedicated Redis HA backend for Envoy global
rate limiting.
...
- OpenTelemetry proxy metrics can target either `envoy.feedyruby.proxy.telemetry.openTelemetry.host`/`port`
  or `backendRefs`, but not both. Prefer `host`/`port` for collectors that live in another namespace.
- When both the main app ingress and the Envoy API ingress are enabled, set `envoy.feedyruby.ingress.host`
...
Cube is part of the baseline FeedyRuby v5 stack and is deployed by this chart by default
(`cube.enabled: true`).

- For the chart-managed Cube, the chart renders `deployment.env.CUBEJS_API_URL` automatically as
  `http://feedyruby-cube:4000` when using the default release name.
...
When the FeedyRuby migration job is enabled, Hub waits for the `feedyruby-migration` Job to complete before its own goose/river init migrations run. This keeps fresh shared-database installs from creating Hub tables before Prisma has initialized the FeedyRuby schema.
...
When deployed with Argo CD, chart-managed Secrets and ExternalSecrets render in sync wave `-2`, and the FeedyRuby and Hub migration hooks run in sync wave `-1`. This lets app and Hub secrets exist before migration jobs start.
...
The chart can optionally deploy a FeedyRuby-provided Qwen runtime through the `vllm-stack` dependency. It is disabled by default so existing installs keep using their current AI provider settings.

To deploy the bundled Qwen/vLLM runtime and automatically point the FeedyRuby app at it:
...
Set `llm.autoConfigureApp=false` to deploy the bundled runtime without injecting FeedyRuby app AI env vars.
...
| deployment.image.repository                                        | string | `"docker.io/amirmpa/feedyruby"`                                           |                                                           |
...
| feedyruby.publicUrl                                               | string | `""`                                                                        |                                                           |
| feedyruby.webappUrl                                               | string | `""`                                                                        |                                                           |
...
| hub.image.digest                                                   | string | `"sha256:7d8633cfa0bfa8becc8ff6596fc707333cd3b11f1284dbacb3d4cbe80a86a436"` | When set, takes precedence over tag (immutable pin).      |
| hub.image.pullPolicy                                               | string | `"IfNotPresent"`                                                            |                                                           |
| hub.image.repository                                               | string | `"ghcr.io/salamruby/hub"`                                                  |                                                           |
| hub.image.tag                                                      | string | `"0.5.0"`                                                                   | Fallback when digest is empty.                            |
| hub.migration.activeDeadlineSeconds                                | int    | `900`                                                                       |                                                           |
| hub.migration.backoffLimit                                         | int    | `3`                                                                         |                                                           |
| hub.migration.ttlSecondsAfterFinished                              | int    | `300`                                                                       |                                                           |
| hub.migration.waitForFeedyRubyMigration.enabled                   | bool   | `true`                                                                      |                                                           |
| hub.migration.waitForFeedyRubyMigration.intervalSeconds           | int    | `5`                                                                         |                                                           |
| hub.migration.waitForFeedyRubyMigration.maxAttempts               | int    | `180`                                                                       |                                                           |
| hub.migration.waitForFeedyRubyMigration.missingJobMaxAttempts     | int    | `12`                                                                        | Consecutive missing Job reads before using DB markers.    |
...
| ingress.hosts[0].host                                              | string | `"k8s.feedyruby.com"`                                                      |                                                           |
| ingress.hosts[0].paths[0].path                                     | string | `"/"`                                                                       |                                                           |
| ingress.hosts[0].paths[0].pathType                                 | string | `"Prefix"`                                                                  |                                                           |
| ingress.hosts[0].paths[0].serviceName                              | string | `"feedyruby"`                                                              |                                                           |
...
| llm.feedyruby.baseUrl                                             | string | `""`                                                                        | Defaults to `http://<release-name>-router-service:<llm.routerSpec.servicePort>/v1`. |
| llm.feedyruby.model                                               | string | `"qwen3-14b-awq"`                                                           | FeedyRuby `AI_MODEL` value for the bundled runtime.      |
| llm.feedyruby.providerName                                        | string | `"vllm"`                                                                    | FeedyRuby OpenAI-compatible provider display name.       |
| llm.feedyruby.supportsStructuredOutputs                           | string | `"1"`                                                                       | Enables structured output usage for the bundled runtime.  |
...
| postgresql.auth.database                                           | string | `"feedyruby"`                                                              |                                                           |
| postgresql.auth.existingSecret                                     | string | `"feedyruby-app-secrets"`                                                  |                                                           |
...
| postgresql.auth.username                                           | string | `"feedyruby"`                                                              |                                                           |
| postgresql.enabled                                                 | bool   | `true`                                                                      |                                                           |
| postgresql.externalDatabaseUrl                                     | string | `""`                                                                        |                                                           |
| postgresql.fullnameOverride                                        | string | `"feedyruby-postgresql"`                                                   |                                                           |
...
| redis.auth.existingSecret                                          | string | `"feedyruby-app-secrets"`                                                  |                                                           |
| redis.auth.existingSecretPasswordKey                               | string | `"REDIS_PASSWORD"`                                                          |                                                           |
| redis.enabled                                                      | bool   | `true`                                                                      |                                                           |
| redis.externalRedisUrl                                             | string | `""`                                                                        |                                                           |
| redis.fullnameOverride                                             | string | `"feedyruby-redis"`                                                        |                                                           |
