...
description: This document outlines the language-specific conventions for the FeedyRuby codebase, providing guidelines for writing code in TypeScript/JavaScript.
icon: code
---
...
2. **Architecture Patterns**
   ...
   - **Singleton Pattern**: Main SDK class (`FeedyRuby`) with static interface
   - **Manager Pattern**: Specialized managers (`UserManager`, `SurveyManager`, `PresentSurveyManager`)
   - **Builder Pattern**: Configuration objects (`FeedyRubyConfig.Builder`)
   - **Protocol-Oriented Programming**: Service protocols for dependency injection and testing

3. **Error Handling**

   - Custom error enums with descriptive cases (`FeedyRubySDKErrorType`)
...
4. **Naming Conventions**
   ...
   - Classes: PascalCase (`FeedyRubyConfig`, `UserManager`)
   - Properties/Methods: camelCase (`workspaceId`, `setUserId`)
   - Constants: camelCase with descriptive names
   - Protocol names: Descriptive with "Protocol" suffix (`FeedyRubyServiceProtocol`)
...
```swift
// Class definitions
@objc(FeedyRuby) public class FeedyRuby: NSObject {
...
@objc public static func setup(with config: FeedyRubyConfig, force: Bool = false)

// Guard statements for early returns
guard !isInitialized else {
    let error = FeedyRubySDKError(type: .sdkIsAlreadyInitialized)
    FeedyRuby.logger?.error(error.message)
    return
}

// Enum cases with descriptive names
public enum FeedyRubySDKErrorType: Int {
...
2. **Kotlin Language Features**
   ...
   - **Object singletons** for stateless utilities and managers (`FeedyRuby`, `Logger`, `SDKError`)
