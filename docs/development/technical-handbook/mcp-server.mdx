---
title: "MCP Server"
description: "Configure and use the FeedyRuby v3 Surveys MCP server"
icon: "robot"
---

## Overview

The FeedyRuby MCP server exposes a small v3 Surveys tool surface for AI agents. It runs inside the
FeedyRuby web app at `/api/mcp` and reuses the same v3 Surveys API authentication, authorization,
...
Use the streamable HTTP MCP endpoint on the FeedyRuby app:

```text
https://app.feedyruby.com/api/mcp
```
...
response headers. Browser-origin MCP requests must come from the configured public FeedyRuby origin.

## Authentication

Authenticate with a FeedyRuby API key in a request header:
...
Start FeedyRuby and prepare the database:

```bash
pnpm db:up
pnpm db:migrate:dev
pnpm --filter @feedyruby/web dev
```

Create an API key in the FeedyRuby app with access to the target workspace. Use the least privileged
permission needed by the agent workflow.

Verify that the MCP endpoint can list tools:

```bash
export FEEDYRUBY_MCP_API_KEY="fbk_..."

curl -sS -X POST http://localhost:3000/api/mcp \
  -H "Accept: application/json, text/event-stream" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $FEEDYRUBY_MCP_API_KEY" \
...
```bash
launchctl setenv FEEDYRUBY_MCP_API_KEY "fbk_..."
```

Register the local MCP server:

```bash
codex mcp add feedyruby-local \
  --url http://localhost:3000/api/mcp \
  --bearer-token-env-var FEEDYRUBY_MCP_API_KEY
```

Equivalent Codex config:

```toml
[mcp_servers.feedyruby-local]
url = "http://localhost:3000/api/mcp"
bearer_token_env_var = "FEEDYRUBY_MCP_API_KEY"
```
...
```bash
codex mcp list
codex mcp get feedyruby-local
```

Then ask Codex to use the configured server:

```text
Use the feedyruby-local MCP server to list surveys for workspace clxx1234567890123456789012.
```
...
```bash
claude mcp add --transport http feedyruby-local http://localhost:3000/api/mcp \
  --header "Authorization: Bearer $FEEDYRUBY_MCP_API_KEY"
```
...
```bash
claude mcp list
claude mcp get feedyruby-local
```
...
```json
{
  "mcpServers": {
    "feedyruby-local": {
      "headers": {
        "Authorization": "Bearer ${FEEDYRUBY_MCP_API_KEY}"
      },
      "type": "http",
      "url": "http://localhost:3000/api/mcp"
    }
...
Claude Code will fail to parse this config if `FEEDYRUBY_MCP_API_KEY` is not set.
