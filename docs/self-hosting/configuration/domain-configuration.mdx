...
description: "Configuring your domain for FeedyRuby."
icon: "globe"
---

FeedyRuby supports both single domain and dual domain configurations with Public/Private Domain Split feature. This guide will help you set up your domains correctly for enhanced security and organization.

## Single Domain Setup

For a single domain setup, you need to configure two essential environment variables:

1. `WEBAPP_URL`: The base URL of your FeedyRuby instance (serves both admin and public content)
2. `NEXTAUTH_URL`: The authentication URL (should be the same as WEBAPP_URL)

### Example Configuration

```bash
WEBAPP_URL=https://feedyruby.example.com
NEXTAUTH_URL=https://feedyruby.example.com
```
...
FeedyRuby supports a powerful domain separation feature that allows you to serve public content (surveys, public APIs) from a different domain than your private admin interface. This provides enhanced security and better organization. It enables you to:
...
**Important Security Note**: FeedyRuby handles application-level domain restrictions to ensure each domain
...
security must be configured by you. FeedyRuby does not manage your infrastructure-level security settings.

### Example Configuration

```bash
# Private admin domain
WEBAPP_URL=https://admin.feedyruby.example.com
NEXTAUTH_URL=https://admin.feedyruby.example.com
...
- `/api/v1/client/{workspaceId}/*` - Client API endpoints (v1)
- `/api/v2/client/{workspaceId}/*` - Client API endpoints (v2)
...
- `/storage/{workspaceId}/{accessType}/{fileName}` - Public file storage access
...
3. **Update your reverse proxy** (nginx, Apache, Cloudflare, etc.) to route both domains to your FeedyRuby instance
4. **Firewall rules**: Ensure both domains can reach your FeedyRuby instance
...
As of now, FeedyRuby does not provide an automated migration between domain setups.
